...
public class FormulaOneDatabase extends SQLiteOpenHelper {
    ...
    private enum DBState {UNKNOWN, INIT, CREATED, UPGRADEDE, OPENED};
    private DBState state = DBState.UNKNOWN;
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS " + "Rangliste" + "(_id INTEGER PRIMARY KEY AUTOINCREMENT, Fahrer TEXT NOT NULL, Team TEXT NOT NULL, Punkte INTEGER, Siege INTEGER);");
        state = DBState.CREATED;
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("DROP TABLE IF EXISTS " + "Rangliste");
        state = DBState.UPGRADEDE;
        onCreate(db);
        fillDB(db);
    }
